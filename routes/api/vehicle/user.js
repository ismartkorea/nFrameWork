...
// --> /api/vehicle/tracker/user
router.get('/', function(req, res, next) {
  d.log("/api/vehicle/tracker/user");
  res.send('/api/vehicle/tracker/user');
});

//// 어시스트 프로 사용자 권한 체크
////
router.get('/auth/service', function(req, res, next) {
  d.log("GET /api/vehicle/tracker/user/auth/service");
  res.render('api/vehicle/tracker/user/auth/service');
});

router.post('/auth/service', function(req, res, next) {
  d.log("POST /api/vehicle/tracker/user/auth/service");
  ...
  var api_res = {
    success: true,
    code: 200,
    desc: "해당 사용자는 자동차 데이터 판독기 서비스 이용중입니다."
  };
  ...
  async.waterfall(tasks, function(err) {
    ...
    if (err) {
      d.log("[error] /api/vehicle/tracker/user/auth/service : " + err.message);
      api_res.success = false;
      api_res.desc = err.message;
      res.send(api_res);
    }
    else {
      d.log('[done] /api/vehicle/tracker/user/auth/service');
      res.send(api_res);
    }
  });
});

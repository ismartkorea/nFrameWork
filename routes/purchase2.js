...
router.get('/:pCode/:pUniqCode', function(req, res) {
    ...
    var SQL1 = 'SELECT p_no as pNo, p_code as pCode, p_nm as pNm, p_price as pPrice, p_uniq_code as pUniqCode, p_smmry as pSmmry,'
        + ' p_desc as pDesc, opt_cnt as optCnt FROM product_info_tbl WHERE p_div = "M" AND p_display_yn = "Y" AND p_code = ?;';
    // 장바구니 리스트 조회.
    var SQL2 = ' SELECT p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
        ...
        + ' p_uniq_code as pUniqCode FROM cart_inf_tbl WHERE usr_id = ? AND p_uniq_code = ? ORDER BY p_code ASC, sort_no ASC;';
    // 장바구니 합계 조회.
    var SQL3 = 'SELECT SUM(p_price) as price FROM cart_inf_tbl WHERE usr_id = ? AND p_uniq_code = ?;';
    // 상품결제 정보 조회.
    var SQL4 = 'SELECT x.order_no as orderNo, x.p_code as pCode, x.p_pair_code_yn as pPairCodeYn,'
        ...
        + ' FROM order_detail_inf_tbl x, pay_info_tbl y WHERE x.order_no = y.order_no'
    ...
    var SQL5 = 'SELECT x.order_no as orderNo, x.use_term_days as useTermDays, @rDays:=use_term_days, DATEDIFF(now(), x.pay_date) as usedDays,'
        + ' DATEDIFF(adddate(x.pay_date, @rDays), now()) as remainDays, x.p_total_price as totalPrice,'
        + ' ROUND(x.p_total_price / @rDays) as dayPrice, ROUND((x.p_total_price / @rDays) * DATEDIFF(adddate(x.pay_date, @rDays), now())) as remainPrice'
        + ' FROM pay_info_tbl x, order_detail_inf_tbl y WHERE x.order_no = y.order_no AND y.p_code = ? AND x.pay_result = "paid" AND x.usr_id = ?;';
    // 결제 정보 승인대기 갯수 조회.
...
router.get('/', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_nm as pNm, p_price as pPrice, p_uniq_code as pUniqCode,'
        + ' p_desc as pDesc, opt_cnt as optCnt FROM product_info_tbl WHERE p_div = "M" AND p_display_yn = "Y";'
        + ' SELECT p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
        + ' CASE WHEN p_div = "M" THEN "기본" WHEN p_div = "O" THEN "옵션" ELSE "" END as pDivNm,'
        + ' p_opt_btn_dis_yn as pOptBtnDisYn, p_nm as pNm, p_price as pPrice, opt_cnt as optCnt,'
        + ' p_uniq_code as pUniqCode FROM cart_inf_tbl WHERE usr_id = ? ORDER BY p_code ASC, sort_no ASC;'
        + 'SELECT SUM(p_price) as price FROM cart_inf_tbl WHERE usr_id = ?;'
        ...
        + ' FROM pay_info_tbl WHERE pay_result = "paid" AND usr_id = ?;'
...
router.post('/opt', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_nm as pNm, p_price as pPrice, p_uniq_code as pUniqCode,'
        ...
        + ' opt_cnt as optCnt, sort_no as sortNo FROM product_info_tbl WHERE p_div = "O" AND p_uniq_code = ?;',
        [pUniqCode],
        function(err, results) {
            if(err) {
                console.log('err : ', err);
...
router.post('/cart/save', function(req, res) {
    ...
    if(pPairCodeYn == 'Y') {
        DEL_SQL1 = 'DELETE FROM cart_inf_tbl WHERE p_code = "' + pPairCode + '" AND p_uniq_code = ? AND usr_id = ?;';
        console.log('>>> DEL_SQL1 : ', DEL_SQL1 + '\n');
    }

    var DEL_SQL2 = 'DELETE FROM cart_inf_tbl WHERE p_code = ? AND p_uniq_code = ? AND usr_id = ?;';
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            // 전체 데이타를 조회한 후 결과를 'results' 매개변수에 저장.
            conn.connect();
            conn.query('DELETE FROM cart_inf_tbl WHERE p_div = "M" AND p_uniq_code = ? AND usr_id = ?',
                [pUniqCode, ss.usrId],
                function (err, results) {
                    if (err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.connect();
            conn.query('INSERT INTO cart_inf_tbl(usr_id, p_code, p_pair_code_yn, p_pair_code, p_div, p_type, p_gubun,'
                ...
                + '"' + ss.usrId + '" FROM product_info_tbl WHERE p_div = "M" AND p_uniq_code = ?',
                [pUniqCode],
                function (err, results) {
                    if (err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('INSERT INTO cart_inf_tbl(usr_id, p_code, p_pair_code_yn, p_pair_code, p_div, p_type, p_gubun,'
                ...
                + 'SELECT "' + ss.usrId + '", p_code, p_pair_code_yn, p_pair_code, p_div, p_type, p_gubun,'
                + ' p_opt_btn_dis_yn, p_nm, p_price, p_uniq_code, sort_no, opt_cnt, now(), "' + ss.usrId + '", now(),'
                + '"' + ss.usrId + '" FROM product_info_tbl WHERE p_div = "O" AND p_code = ? AND p_uniq_code = ?' ,
                [pCode, pUniqCode],
                function (err, results) {
                    if (err) {
                        console.log('err : ', err);
...
router.post('/cart/delete', function(req, res) {
    ...
    async.waterfall([
        function(callback) {
            ...
            if(pDiv == "M") {
                ...
                var conn = mysql.createConnection(config);
                conn.connect();
                conn.query('DELETE FROM cart_inf_tbl WHERE p_uniq_code = ? AND usr_id = ?',
                    [pUniqCode, ss.usrId],
                    function(err, results) {
                        if(err) {
                            console.log('err : ', err);
    ...
    var conn = mysql.createConnection(config);
    conn.connect();
    conn.query('DELETE FROM cart_inf_tbl WHERE p_code = ? AND p_uniq_code =?',
        [pCode, pUniqCode],
        function(err, results) {
            if(err) {
                console.log('err : ', err);
...
router.post('/cart/alldel', function(req, res) {
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM cart_inf_tbl WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
    ...
    async.waterfall([
        function(callback) {
            if(prevOrderNo != '') {
                ...
                var conn = mysql.createConnection(config);
                conn.connect();
                conn.query('INSERT INTO pay_remain_inf_tbl(order_no, usr_id, remain_days, used_days, remain_price,'
        ...
        function(callback) {
            // 기존 주문 히스토리에 복사.
            var conn = mysql.createConnection(config);
            conn.query('INSERT INTO  order_detail_his_inf_tbl  (SELECT * FROM order_detail_inf_tbl WHERE p_uniq_code = ? AND usr_id = ?);'
                + 'INSERT INTO  order_his_inf_tbl  (order_no, usr_id, service_nm, total_price, order_date, use_term_days,'
                ...
                + ' x.insert_usr, x.update_dt, x.update_usr FROM order_inf_tbl x, order_detail_inf_tbl y '
        ...
        function(callback) {
            ...
            // MySQL Connect
            var conn = mysql.createConnection(config);
            conn.query('INSERT INTO order_detail_inf_tbl(order_no, usr_id, p_code, p_pair_code_yn, p_pair_code,'
                + 'p_div, p_nm, p_opt_btn_dis_yn, p_price, p_uniq_code, sort_no, opt_cnt, insert_dt, insert_usr, update_dt, update_usr)'
                + ' SELECT "' + orderNum + '", "' + ss.usrId + '", p_code, p_pair_code_yn, p_pair_code,'
                + 'p_div, p_nm, p_opt_btn_dis_yn, p_price, p_uniq_code, sort_no, opt_cnt, now(), "' + ss.usrId + '", now(), "' + ss.usrId + '"'
                + 'FROM cart_inf_tbl WHERE p_uniq_code = ? AND usr_id = ?',
        ...
        function(callback) {
            ...
            console.log('totalCalPayPrice : ', totalCalPayPrice + '\n');
            console.log('useTermDays : ', useTermDays + '\n');
            var utDays = useTermDays !=null ? parseInt(useTermDays) : 0;
            console.log('utDays : ', utDays + '\n');

            // 주문 처리.
            var conn = mysql.createConnection(config);
            conn.query('INSERT INTO order_inf_tbl(order_no, usr_id, service_nm, total_price, order_date, use_term_days,'
                + ' insert_dt, insert_usr, update_dt, update_usr)'
                + 'SELECT MAX("' + orderNum + '"), "' + ss.usrId + '", (SELECT MAX(p_nm) as p_nm FROM cart_inf_tbl WHERE p_code = ? AND p_div = "M" AND usr_id = ?),'
                + '"' + totalCalPayPrice + '", now(), "' + utDays + '", now(), "' + ss.usrId + '", now(), "' + ss.usrId
                + '" FROM cart_inf_tbl WHERE p_code = ? AND p_div = "M" AND usr_id = ?;',
        ...
        function(callback) {
            ...
            conn.query('SELECT order_no as orderNo, p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
                + ' CASE WHEN p_div = "M" THEN "기본" WHEN p_div = "O" THEN "옵션" ELSE "" END as pDivNm,'
                + ' p_opt_btn_dis_yn as pOptBtnDisYn, p_nm as pNm, p_price as pPrice, opt_cnt as optCnt,'
                + ' p_uniq_code as pUniqCode FROM order_detail_inf_tbl WHERE order_no = ? AND usr_id = ? ORDER BY p_code ASC, sort_no ASC;'
                + ' SELECT order_no as orderNo, service_nm as serviceNm, DATE_FORMAT(order_date, "%Y-%m-%d") as orderDate'
                + ' FROM order_inf_tbl WHERE order_no = ? AND usr_id = ?;',
...
router.post('/order/delete', function(req, res) {
    ...
    if(pPairCodeYn == 'Y') {
        DEL_SQL1 = 'DELETE FROM order_detail_inf_tbl WHERE p_code = "' + pPairCode + '" AND p_uniq_code = ? AND usr_id = ?;';
        console.log('>>> DEL_SQL1 : ', DEL_SQL1 + '\n');
    }

    var DEL_SQL2 = 'DELETE FROM order_detail_inf_tbl WHERE p_code = ? AND p_uniq_code = ? AND usr_id = ?;';
    console.log('>>> DEL_SQL2 : ', DEL_SQL2 + '\n');
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            if(pDiv == "M") {
                ...
                var conn = mysql.createConnection(config);
                conn.connect();
                conn.query('DELETE FROM order_detail_inf_tbl WHERE p_uniq_code = ? AND usr_id = ?',
                    [pUniqCode, ss.usrId],
                    function(err, results) {
                        if(err) {
                            console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_inf_tbl WHERE order_no = ?',
                [pUniqCode, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
    ...
    var conn = mysql.createConnection(config);
    conn.connect();
    conn.query('DELETE FROM order_detail_inf_tbl WHERE p_code = ? AND p_uniq_code =? AND order_no = ?',
...
router.post('/order/alldel', function(req, res) {
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_detail_inf_tbl WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_inf_tbl WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('SELECT order_no as orderNo, p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
                + ' CASE WHEN p_div = "M" THEN "기본" WHEN p_div = "O" THEN "옵션" ELSE "" END as pDivNm,'
                + ' p_opt_btn_dis_yn as pOptBtnDisYn, p_nm as pNm, p_price as pPrice, opt_cnt as optCnt,'
                + ' p_uniq_code as pUniqCode FROM order_detail_inf_tbl WHERE usr_id = ? ORDER BY p_code ASC, sort_no ASC;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
...
router.post('/calculate', function(req, res) {
    ...
    var arrPcode = []; var arrPdiv = []; var arrPuniqCode = []; var arrPprice = []; var arrPoptCnt = [];
    var totalCalPrice = 0; var getOptCnt = 0; var optCnt = 0; var calPrice = 0; var disCountPct = 0;
...
router.get('/paypopup', function(req, res, next) {
    ...
    // product_info 상품 조회.
    var SQL1 = 'SELECT p_no as pNo, p_code as pCode, p_nm as pNm, p_price as pPrice, p_uniq_code as pUniqCode,'
        + ' p_desc as pDesc, opt_cnt as optCnt FROM product_info_tbl WHERE p_div = "M" AND p_display_yn = "Y";';
    // 주문 조회.
/*
    var SQL2 = ' SELECT p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
    ...
    var SQL2 = 'SELECT order_no as orderNo, usr_id as usrId, service_nm as serviceNm, total_price as totalPrice,'
        ...
        + 'FROM order_inf_tbl WHERE usr_id = ? ORDER BY order_date DESC;';
    var SQL2 = 'SELECT order_no as orderNo, usr_id as usrId, p_code as pCode, p_div as pDiv,'
        ...
        + ' p_opt_btn_dis_yn as pOptBtnDisYn, opt_cnt as optCnt FROM order_detail_inf_tbl WHERE usr_id = ? ORDER BY p_code ASC, sort_no ASC;';
*/
    var SQL2 = 'SELECT x.order_no as orderNo, x.usr_id as usrId, x.p_code as pCode, x.p_div as pDiv,'
            ...
            + ' (SELECT total_price FROM order_inf_tbl  WHERE order_no = x.order_no AND x.p_div = "M" AND x.usr_id = ?) as totalPrice,'
            + ' x.p_uniq_code as pUniqCode, x.p_opt_btn_dis_yn as pOptBtnDisYn, x.opt_cnt as optCnt'
            + ' FROM order_detail_inf_tbl x, order_inf_tbl y WHERE x.order_no = y.order_no AND y.pay_result IS NULL'
            + ' AND y.usr_id = ? ORDER BY x.p_code ASC, x.sort_no ASC;';

    // 카트 합계 조회.
    var SQL3 = 'SELECT SUM(total_price) as price FROM order_inf_tbl WHERE pay_result IS NULL AND usr_id = ?;';
    // 이전 서비스 내역 조회.
    var SQL4 = 'SELECT x.order_no as orderNo, x.p_code as pCode, x.p_pair_code_yn as pPairCodeYn, x.p_div as pDiv,'
        + ' x.p_opt_btn_dis_yn as pOptBtnDisYn, x.p_nm as pNm, x.p_price as pPrice, x.p_uniq_code as pUniqCode,'
        + ' x.sort_no as sortNo, y.pay_result as payResult'
        + ' FROM order_detail_inf_tbl x, pay_info_tbl y WHERE x.order_no = y.order_no'
        + ' AND x.usr_id = y.usr_id AND y.pay_result = "paid" AND y.usr_id = ? ORDER BY x.p_code ASC, x.sort_no ASC;';
    ...
    var conn = mysql.createConnection(config);
    conn.connect();
    conn.query(SQL1+SQL2+SQL3+SQL4+SQL5, [usrId, usrId, usrId, usrId, usrId], function(err, results) {
            if(err) {
                console.log('err : ', err);
            } else {
                var cartPrice = 0;
...
router.post('/paypopup/list', function(req, res, next) {
    ...
    var usrId = ss.usrId !=null ? ss.usrId : '';
    // product_info 상품 조회.
    var SQL1 = 'SELECT x.order_no as orderNo, x.usr_id as usrId, x.p_code as pCode, x.p_div as pDiv,'
        + ' CASE WHEN x.p_div = "M" THEN "기본" WHEN x.p_div = "O" THEN "옵션" ELSE "" END as pDivNm,'
        + ' x.p_pair_code_yn as pPairCodeYn, x.p_nm as pNm, x.p_price as pPrice, y.use_term_days as useTermDays,'
        + ' (SELECT total_price FROM order_inf_tbl  WHERE order_no = x.order_no AND x.p_div = "M" AND x.usr_id = ?) as totalPrice,'
        + ' x.p_uniq_code as pUniqCode, x.p_opt_btn_dis_yn as pOptBtnDisYn, x.opt_cnt as optCnt'
    ...
    // 카트 합계 조회.
    var SQL2 = 'SELECT IFNULL(SUM(total_price),0) as price FROM order_inf_tbl WHERE pay_result IS NULL AND usr_id = ?;';
...
router.post('/paypopup/alldel', function(req, res) {
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_detail_his_inf_tbl WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_his_inf_tbl WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('SELECT x.order_no as orderNo, x.usr_id as usrId, x.p_code as pCode, x.p_div as pDiv,'
                + ' CASE WHEN x.p_div = "M" THEN "기본" WHEN x.p_div = "O" THEN "옵션" ELSE "" END as pDivNm,'
                + ' x.p_pair_code_yn as pPairCodeYn, x.p_nm as pNm, x.p_price as pPrice, y.use_term_days as useTermDays,'
                + ' (SELECT total_price FROM order_inf_tbl  WHERE order_no = x.order_no AND x.p_div = "M" AND x.usr_id = ?) as totalPrice,'
                + ' x.p_uniq_code as pUniqCode, x.p_opt_btn_dis_yn as pOptBtnDisYn, x.opt_cnt as optCnt'
                + ' FROM order_detail_inf_tbl x, order_inf_tbl y WHERE x.order_no = y.order_no AND y.usr_id = ? ORDER BY x.p_code ASC, x.sort_no ASC;'
                + ' SELECT IFNULL(SUM(total_price),0) as totalPrice FROM order_inf_tbl WHERE usr_id = ?;',
...
router.post('/paypopup/delete', function(req, res) {
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_inf_tbl WHERE order_no = ? AND usr_id = ?;',
                [orderNo, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_detail_his_inf_tbl WHERE p_uniq_code = ? AND usr_id = ?;',
                [pUniqCode, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM order_his_inf_tbl WHERE order_no = ? AND usr_id = ?;',
                [orderNo, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('SELECT x.order_no as orderNo, x.usr_id as usrId, x.p_code as pCode, x.p_div as pDiv,'
                ...
                + ' FROM order_detail_inf_tbl x, order_inf_tbl y WHERE x.order_no = y.order_no AND y.pay_result IS NULL'
                + ' AND y.usr_id = ? ORDER BY x.p_code ASC, x.sort_no ASC;'
                + ' SELECT IFNULL(SUM(total_price),0) as totalPrice FROM order_inf_tbl WHERE pay_result IS NULL AND usr_id = ?;',
                [ss.usrId, ss.usrId, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
